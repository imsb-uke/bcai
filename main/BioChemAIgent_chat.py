...
from src.history import save_history, load_history, message_to_dict, HISTORY_FILE, HISTORY_DIR
...
# Config files (each may contain multiple servers under "mcpServers")
SERVER_CONFIG_FILES = [
    "../config/ChEMBL_MCP_Server.json",
    # "../config/PDB-MCP-Server.json",
...
async def process_query(query: str) -> str:
    ...
    messages = [{"role": "system", "content": system_message}] + load_history()
...
async def main() -> None:
    ...
    print(f"Version: 0.5.0")
    print(f"LLM model: {model_full}")
    print("=========================\n")
    ...
    while True:
        try:
            # query = input("\nUser: ").strip()
            query = input("\n\033[1;34mUser:\033[0m ").strip()
            # \033[ starts an ANSI escape sequence, 1 → bold, 34 → blue, 0m → reset to default color
            if not query:
                continue
            ...
            if query.lower() == "save":
                history_name = input("\nChat name to save: ").strip() + ".pkl"
                os.system(f"cp {HISTORY_FILE} {os.path.join(HISTORY_DIR, history_name)}")
                print(f"The chat is saved as {history_name}")
                continue

            if query.lower() == "load":
                history_name = input("\nChat name to load: ").strip() + ".pkl"
                src = os.path.join(HISTORY_DIR, history_name)
                if os.path.exists(src):
                    os.system(f"cp {src} {HISTORY_FILE}")
                    print(f"The chat is loaded from {history_name}")
                else:
                    print(f"The chat history {history_name} does not exist!")
                continue

            # Get response from Agent. We use a while loop as some LLMs sometimes generate empty outputs.
            response = ''
            while response == '':
                response = await process_query(query)
                if response == '':
                    print('Processing ...')
            print("\n\033[1;31mAgent:\033[0m " + response)
