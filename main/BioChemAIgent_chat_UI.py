...
JOB_READY_FILE = os.path.join(SESSION_DIR, "job_ready.json")  # claude: flag written by client.py when an async job completes
...
# claude ---
def job_ready_changed(path: str) -> bool:
    """Check if client.py has written a job-done flag since last check."""
    if not os.path.exists(path):
        return False

    last = st.session_state.get("job_ready_mtime", 0.0)
    current = os.path.getmtime(path)

    if current != last:
        st.session_state["job_ready_mtime"] = current
        return True
...
# claude ---
# Check for job completion on every rerun (passive — no polling loop).
# client.py writes job_ready.json when a background job finishes; we read it here,
# store the message, delete the file, and rerun once so the banner below appears.
