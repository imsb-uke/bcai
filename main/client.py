...
# claude ---
# Pending async jobs: when a tool returns a job_id, we store it here so the
# main loop can poll for completion and write a flag file to notify the UI.
# key = job_id, value = {user_name, session_dir, flag_file}
_pending_jobs: Dict[str, Any] = {}
_seen_job_ids: set = set()   # all job_ids ever registered; prevents re-registering completed ones
# ---
    ...
    for cfg_path in config_files:
        for server_key, server_def in utils.load_servers_from_config(cfg_path):
            ...
            if list_tools:
                ...
                os.system(f"echo 'Connected to {server_key} with {len(tool_names)} tools: {tool_names}' >> client.log")
...
# claude ---
def _extract_new_job_ids(messages: List[Dict]) -> List[str]:
    """
    Scan tool messages for job_ids returned by async tools (status = 'running').
    Called after each query to register new jobs for background polling.
    """
    ids = []
    for msg in messages:
        if msg.get('role') == 'tool':
            try:
                ...
                if 'job_id' in content and content.get('status') == 'running':
                    ids.append(content['job_id'])
            except Exception:
                pass
    return ids
# ---
...
async def main() -> None:
    ...
    while True:
        ...
        # claude ---
        # Poll all pending async jobs; write a flag file to session/<user>/ when one completes.
        # The UI watches that flag file and shows a notification to the user.
        for jid in list(_pending_jobs.keys()):
            try:
                server_key, bare_tool = route('BioChemAIgent_MCP_Server__bcai__check_status')
                raw = await mcp_call(server_key, bare_tool, {'job_id': jid})
                ...
                if result.get('status') == 'done':
                    ...
                    os.system(f"echo '{datetime.now()} | Job {jid} failed: {err}' >> client.log")
                elif result.get('status') == 'cancelled':
                    _pending_jobs.pop(jid)
                    os.system(f"echo '{datetime.now()} | Job {jid} cancelled — removed from pending' >> client.log")
                elif 'No job found' in result.get('message', ''):  # claude: stale job from a previous server session
                    _pending_jobs.pop(jid)
                    os.system(f"echo '{datetime.now()} | Job {jid} not found (stale) — removed from pending' >> client.log")
            except Exception as e:
                os.system(f"echo '{datetime.now()} | Error polling job {jid}: {e}' >> client.log")
        # ---
        if file and os.path.getsize(file) > 0:
            ...
            try:
                ...
                # claude: seed _seen_job_ids with all historical job IDs so they
                # are never re-registered after a restart — only jobs started in
                # this session will be picked up after process_query returns.
                for jid in _extract_new_job_ids(load_history(file)):
            ...
            # claude ---
            # Register any new async job_ids so the loop below can poll for completion.
            # Historical IDs were already seeded into _seen_job_ids above, so only
            # jobs started in this query round will pass the guard below.
            new_ids = _extract_new_job_ids(load_history(session_info['history_file']))
            for jid in new_ids:
                if jid not in _seen_job_ids:   # guard against re-registering completed jobs
                    _seen_job_ids.add(jid)
                    _pending_jobs[jid] = {
                        'user_name'   : user_name,
                        'session_dir' : session_dir,
                        'flag_file'   : os.path.join(session_dir, 'job_ready.json'),
                    }
                    os.system(f"echo '{datetime.now()} | Pending job registered: {jid} for {user_name}' >> client.log")
            # ---
