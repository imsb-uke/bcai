...
from biochem.dock_utilities import run_docking, extract_score  # claude: called per-compound below so stop_event can be checked between compounds; biochem itself has no notion of jobs/threading
...
def run_virtual_screening(
    ...
    against the prepared protein, one compound at a time.

    Runs the per-compound docking loop in an internal thread so the output
    directory can be polled every 5 s for progress. The loop itself lives here
    (not in biochem.run_molecular_docking) so stop_event can be checked between
    compounds — biochem's docking functions take no stop_event / job concept.
    ...
    Returns a dict shaped like biochem.run_molecular_docking()'s old return value,
    but with a real per-compound score (keyed by ligand_name) instead of only the
    last compound docked, augmented with project_name and result_dir.
    """
    library_dir = os.path.join(LIBRARY_DIR, library_name)  # claude
    ...
    def _dock():
        try:
            ...
            df_query = pd.read_csv(query["save_dir"], dtype=str)

            scores     = {}
            pose_files = {}

            for i in range(len(df_query)):
                if stop_event and stop_event.is_set():  # claude: cancel between compounds
                    break

                protein_name = df_query["protein_name"][i]
                ligand_name  = df_query["ligand_name"][i]
                dock_pdbqt_dir = os.path.join(
                    result_dir, f"dock_{docking_method}_{protein_name}_{ligand_name}.pdbqt"
                )
                dock_log_dir = os.path.join(
                    result_dir, f"dock_{docking_method}_{protein_name}_{ligand_name}.log"
                )

                if not os.path.exists(dock_pdbqt_dir):
                    query_dict = {
                        "protein_pdbqt_dir":   df_query["protein_pdbqt_file"][i],
                        "ligand_pdbqt_dir":    df_query["ligand_pdbqt_file"][i],
                        "constraint_file_dir": df_query["constraint_file"][i],
                        "output_pdbqt_dir":    dock_pdbqt_dir,
                        "output_log_dir":      dock_log_dir,
                    }
                    run_docking(
                        query_dict, use_docker=use_docker, method=docking_method,
                        n_cpu="20", exh=exhaustiveness,
                    )

                if not os.path.exists(dock_pdbqt_dir):
                    continue  # docking failed to produce output for this compound — skip scoring it

                pose_dir = os.path.join(
                    result_dir, f"{docking_method}_{protein_name}_{ligand_name}_pose_.sdf"
                )
                os.system(f"obabel {dock_pdbqt_dir} -O {pose_dir} -m > /dev/null 2>&1")

                scores[ligand_name]     = extract_score(dock_pdbqt_dir, docking_method)
                pose_files[ligand_name] = pose_dir

            docking_result[0] = {
                "message":                 "The docking result is ready" if scores else "No compounds were docked.",
                "next_steps":              ["protein_ligand_interaction"],
                "docked ligand sdf files": pose_files,
